...
impl RpcFuncSignature {
    fn common_args_names(&self) -> Vec<syn::Ident> {
        self.common_args.clone().into_iter().map(|a| a.name).collect()
    } 
    ...
    fn server_args_names(&self) -> Vec<syn::Ident> {
        self.server_args.clone().into_iter().map(|a| a.name).collect()
    } 
}
...
impl Parse for RpcFuncSignature {
    fn parse(input: ParseStream) -> Result<Self> {
        ...
        let server_args = args.clone().into_iter()
            .filter(|a| a.server_arg).collect();

        let common_args = args.clone().into_iter()
            .filter(|a| !a.server_arg).collect();
    ...
    fn parse(input: ParseStream) -> Result<Self> {
        let server_arg = if input.peek(Token![#]) {
            ...
            if indent.to_string() != "server" {
                return Err(Error::new_spanned(indent, "only `server` attribute supported"))
            }
            if !content.is_empty() {
...
impl Parse for RpcArgs {
    fn parse(input: ParseStream) -> Result<Self> {
        ...
        let uri = uri
            .ok_or(Error::new_spanned(comma.clone(), "must have an `uri` argument"))?;
        let service = service
            .ok_or(Error::new_spanned(comma.clone(), "must have a `service` argument"))?;
...
impl Parse for RpcArg {
    fn parse(input: ParseStream) -> Result<Self> {
        ...
        match &*argname.to_string() {
            ...
            _ => {
                Err(Error::new_spanned(argname, "expected `uri` or `service`"))
            }
        }
    }
}
...
pub fn rpc(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let RpcFunc {
        ...
        sig,
        block
    } = parse_macro_input!(item as RpcFunc);
